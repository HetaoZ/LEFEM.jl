...
create_elem_type_and_eval(:Hex8)

"""
Hex8's strain matrix is a constant matrix.
"""
function strain_matrix(elem::Hex8, nodes, r, s)
    x = elem_x(elem, nodes)
    x1 = x[1,1]; y1 = x[1,2]
    x2 = x[2,1]; y2 = x[2,2]
    x3 = x[3,1]; y3 = x[3,2]
...
function elem_stress(elem::Hex8)

end

function elem_strain(elem::Hex8)

end

function elem_jacobi(elem::Hex8, nodes)
    x = elem_x(elem, nodes)
    x1 = x[1,1]; y1 = x[1,2]
    x2 = x[2,1]; y2 = x[2,2]
    x3 = x[3,1]; y3 = x[3,2]
...
t: thickness = 1
"""
function integ_elem_elast_brick(elem::Hex8, nodes, E, ν)
...
"""
function integ_elem_mass_brick(elem::Hex8, nodes, ρ, t)
    A = elem_area(elem, nodes)
    Me = ρ * A * t / 4 * ones(Float64,8)
    return Me
end

function integ_elem_brick(elem::Hex8, nodes, para)
